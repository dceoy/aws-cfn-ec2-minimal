...
- `ec2-linux.cfn.yml` launches the Amazon ECS-optimized Amazon Linux 2023 arm64 EC2 instance into that existing infrastructure.
...
The Windows template installs [winget](https://github.com/microsoft/winget-cli) and several desktop-oriented tools, while the Linux template uses a shell bootstrap to update the instance, install `aws-cfn-bootstrap`, and ensure SSM Agent and ECS are running.
...
| `ec2-linux.cfn.yml`   | Amazon ECS-optimized Amazon Linux 2023 arm64 EC2 instance       |
...
| `Ec2SecurityGroupId`        | `''`                              | Optional security group ID override; empty imports `${SystemName}-${EnvType}-ec2-sg`              |
| `Ec2IamInstanceProfileName` | `''`                              | Optional instance profile override; empty imports `${SystemName}-${EnvType}-iam-instance-profile` |
| `Ec2InstanceType`           | `m7g.large`                       | EC2 instance type                                                                                 |
| `Ec2LinuxAmiId`             | Latest ECS-optimized AL2023 arm64 | SSM parameter for the AMI ID                                                                      |
| `Ec2VolumeSize`             | `64`                              | Root EBS volume size in GiB                                                                       |
| `Ec2VolumeType`             | `gp3`                             | Root EBS volume type                                                                              |
| `Ec2UserData`               | Built-in shell bootstrap          | EC2 UserData script before the template appends the CloudFormation signal step                    |
...
- For `ec2-linux.cfn.yml`, custom values should provide the shell commands to run before the template appends the `cfn-signal` step. If custom error handling is used, keep `user_data_exit_code` set so the final signal reports the correct status.
